...
class CategoryController extends Controller
{
    ...
    public function status($id)
    {
        ...
        if ($row == NULL) {
            return redirect()->route('category.index')->with('message', ['type' => 'danger', 'mgs' => 'Thay đổi trạng thái không thành công']);
        }
    ...
    public function delete($id)
    {
        $row = Category::find($id);
        if ($row == NULL) {
            return redirect()->route('category.index')->with('message', ['type' => 'danger', 'mgs' => 'Xóa không thành công']);
        } else {
            $row->status = 0;
            $row->updated_at = date('Y-m-d H:i:s');
            $row->updated_by = 1;
            ...
            return redirect()->route('category.index')->with('message', ['type' => 'success', 'mgs' => 'Xóa thành công']);
        }
    }
    /**
