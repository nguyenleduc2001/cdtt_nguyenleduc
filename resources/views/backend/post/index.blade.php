            ...
            <tbody>
              @foreach ($list as $row)
                  <tr>
                    ...
                    <td>
                      @if ($row->status == 2 )
                        <a href="{{ route('topic.status', ['topic' => $row->id]) }}"class="btn btn-sm btn-success">
                          <i class="fas fa-toggle-on"></i></a>
                      @else
                      <a href="{{ route('topic.status', ['topic' => $row->id]) }}"class="btn btn-sm btn-danger">
                       <i class="fas fa-toggle-off"></i></a>
                      @endif
                      <a href="{{ route('post.edit',['post' => $row ->id]) }}" class="btn btn-sm btn-info" > <i class="fas fa-wrench"></i></a>
                      <a href="{{ route('post.show',['post' => $row ->id]) }}" class="btn btn-sm bg-primary "><i class="far fa-eye"></i></a>
